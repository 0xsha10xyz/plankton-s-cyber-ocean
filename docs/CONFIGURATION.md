...
**Example:** Live PAP **holder count** is served as `GET /api/market/token-info?mint=<MINT>&holders=1` (returns `{ mint, holderCount }` from Jupiter’s index), not as a separate endpoint. This keeps you within the function budget.
...
### Vercel site plus VPS Agent (Claude): setup order (copy/paste)
