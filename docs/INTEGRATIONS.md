...
| **OOBE Protocol** | **Phase 1:** **`GET /api/oobe/status`**, **`GET /api/oobe/probe`** (RPC + agent wallet). **Phase 2 (opt-in):** on-chain Merkle memory via `oobe-protocol` when **`OOBE_MEMORY_ENABLED=1`** — hooks **`POST /api/agent/chat`** (async, costs SOL) and **`POST /api/oobe/memory`** (manual test). | **`OOBE_AGENT_PRIVATE_KEY`**, **`SOLANA_RPC_URL`**, **`OPENAI_API_KEY`** or **`OOBE_OPENAI_API_KEY`** (or **`ANTHROPIC_API_KEY`** / **`GROQ_API_KEY`** for phase-1 readiness only). Phase 2: **`OOBE_MEMORY_ENABLED=1`**. Optional Merkle seeds / **`OOBE_KEY`**. Docs: [OOBE Protocol](https://oobe-protocol.gitbook.io/oobe-protocol). |
