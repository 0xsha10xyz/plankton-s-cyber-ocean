...
| **Hive Protocol** | Task marketplace + agent profile via `@luxenlabs/hive-agent` on the VPS; Dashboard **Hive** tab calls same-origin `/api/hive/*` | `HIVE_API_KEY` (and optional `HIVE_API_BASE_URL`) only in **`backend/.env`** on the VPS. On Vercel set **`AGENT_BACKEND_ORIGIN`**; **`vercel.json`** rewrites `/api/hive/*` into **`api/agent/index`** (repo root) or **`frontend/api/agent/config`** (Root Directory **`frontend`**) — shared **`server-lib/hive-proxy.ts`** so Hobby stays within the **12 serverless function** limit. Maintainer doc: **[Hive integration](./hive-integration.md)**. Upstream: [Hive Protocol](https://uphive.xyz/docs). |
| **OOBE Protocol** | **Phase 1:** **`GET /api/oobe/status`**, **`GET /api/oobe/probe`**. **Phase 2 (opt-in):** on-chain Merkle memory when **`OOBE_MEMORY_ENABLED=1`** — async hook on **`POST /api/agent/chat`**, manual **`POST /api/oobe/memory`**, UI banner on **`/agent-chat`**. | **`OOBE_AGENT_PRIVATE_KEY`**, **`SOLANA_RPC_URL`**, **`OOBE_OPENAI_API_KEY`** or **`OPENAI_API_KEY`** (phase 2). Phase 1 readiness also accepts **`ANTHROPIC_API_KEY`** / **`GROQ_API_KEY`**. See **[OOBE integration](./oobe-integration.md)**. Upstream: [OOBE Protocol](https://oobe-protocol.gitbook.io/oobe-protocol). |
...
| [OOBE integration](./oobe-integration.md) | On-chain agent memory (OOBE Protocol), env, API, UI banner, verification |
