...
| **OOBE Protocol** (phase 1) | Readiness + RPC probe for a dedicated agent wallet on Solana; **`GET /api/oobe/status`** and **`GET /api/oobe/probe`** on the VPS. **`GET /api/agent/config`** includes an **`oobe`** object. Does **not** install `oobe-protocol` yet (heavy SDK). | **`OOBE_AGENT_PRIVATE_KEY`** (or `OOBE_PRIVATE_KEY`), **`SOLANA_RPC_URL`**, **`OOBE_OPENAI_API_KEY`** or reuse **`OPENAI_API_KEY`**. Optional: **`OOBE_AGENT_PUBKEY`**, **`OOBE_KEY`**, **`OOBE_MERKLE_DB_SEED`**, **`OOBE_MERKLE_ROOT_SEED`**, **`OOBE_ENABLED=0`** to disable. Docs: [OOBE Protocol](https://oobe-protocol.gitbook.io/oobe-protocol). |
