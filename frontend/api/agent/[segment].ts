 ...
 *
 * NOTE: This file is duplicated under `frontend/api/` so Vercel projects whose Root Directory is `frontend/`
 * will still deploy these serverless routes (otherwise `/api/agent/*` can fall back to the SPA).
 */
...
async function handleLogs(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (!rpcUrl || slot == null) {
    sendJson(
      res,
      200,
      { source: "stub", lines: [], error: lastErr || "RPC unavailable" },
