...
const NAV_CONFIG: { label: string; sectionId: string }[] = [
...
const Header = () => {
  ...
  const [activeSection, setActiveSection] = useState<string>(NAV_CONFIG[0].sectionId);
  ...
  const updateActiveSection = useCallback(() => {
    ...
    let current = NAV_CONFIG[0].sectionId;

    for (let i = NAV_CONFIG.length - 1; i >= 0; i--) {
      const el = document.getElementById(NAV_CONFIG[i].sectionId);
      if (el) {
        const top = el.getBoundingClientRect().top;
        if (top <= headerOffset) {
          current = NAV_CONFIG[i].sectionId;
          break;
        }
      }
    }
      ...
      <header className="fixed top-0 left-0 right-0 z-50 glass-card border-b border-border/50">
        <div className="container mx-auto flex items-center justify-between px-4 py-3">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {NAV_CONFIG.map(({ label, sectionId }) => (
              <motion.button
                key={sectionId}
                type="button"
