...
/** Get decimals for a mint via RPC getTokenSupply (robust fallback for some RPC/account parser edge cases). */
async function getDecimalsRpcTokenSupply(mint: string): Promise<number | null> {
  try {
    const rpcUrl = process.env.SOLANA_RPC_URL || "https://api.mainnet-beta.solana.com";
    const res = await fetch(rpcUrl, {
      method: "POST",
...
marketRouter.get("/token-info", async (req: Request, res: Response) => {
  ...
  if (apiKey) {
    try {
      ...
      if (resp.ok) {
        ...
        const symbol = typeof data?.symbol === "string" ? data.symbol.trim() : undefined;
        ...
        if (decimals !== undefined) {
          ...
          const displaySymbol = symbol || name || truncated;
          res.json({ symbol: displaySymbol, name: name || undefined, decimals });
          return;
        }
      }
    } catch {
  ...
  for (const jupUrl of jupiterSearchUrls) {
    try {
      ...
      if (obj && typeof obj.decimals === "number" && obj.decimals >= 0 && obj.decimals <= 18) {
        ...
        const truncated = mint.slice(0, 4) + "…" + mint.slice(-4);
        const symbol = sym || nm || truncated;
        res.json({ symbol, name: nm || undefined, decimals: obj.decimals });
        return;
      }
    } catch {
      // try next URL
  ...
  if (meta && (meta.symbol || meta.name)) {
    ...
    if (decimals !== null && decimals >= 0 && decimals <= 18) {
      const truncated = mint.slice(0, 4) + "…" + mint.slice(-4);
      const raw = (meta.symbol || meta.name || "").trim();
      const symbol = raw || truncated;
  ...
  // Robust fallback: getTokenSupply returns decimals for valid mint accounts.
  try {
    const decimals = await getDecimalsRpcTokenSupply(mint);
    if (decimals !== null) {
      res.json({ symbol: mint.slice(0, 4) + "…" + mint.slice(-4), decimals });
      return;
    }
